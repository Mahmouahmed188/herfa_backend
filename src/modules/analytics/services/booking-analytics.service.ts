...
export class BookingAnalyticsService {
  ...
  async getAnalytics(
    ...
    const totalBookings = await this.bookingRepository.count({
      where: { createdAt: { $between: [range.startDate, range.endDate] } as any },
    });
    ...
    for (const row of statusDistributionRaw) {
      statusDistribution[row.status] = totalForStatus > 0
    ...
    const conversionRate = conversionResult && parseInt(conversionResult.total) > 0
      ? Number((parseInt(conversionResult.completed) / parseInt(conversionResult.total) * 100).toFixed(1))
      : 0;
    ...
    return {
      totalBookings,
      dailyBookings: dailyBookings.map((d) => ({ date: d.date, count: parseInt(d.count) })),
      weeklyBookings: weeklyBookings.map((w) => ({ date: w.week, count: parseInt(w.count) })),
      monthlyBookings: monthlyBookings.map((m) => ({ date: m.month, count: parseInt(m.count) })),
      statusDistribution: {
