...
describe('ProviderVerificationAdminService', () => {
  ...
  const createModule = async (overrides?: {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: overrides?.profileRepo ?? {
            findOne: jest.fn(),
            update: jest.fn(),
          },
        },
    ...
    return module.get<ProviderVerificationAdminService>(
      ProviderVerificationAdminService,
    );
  };
  ...
  describe('approve', () => {
    it('should approve verification successfully', async () => {
      const verificationRepo = {
        ...
        save: jest
          .fn()
          .mockResolvedValue(mockVerification({ status: 'approved' })),
      };
      const profileRepo = {
    ...
    it('should throw NotFoundException when verification not found', async () => {
      ...
      await expect(service.approve('invalid-id', mockAdminId)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw BadRequestException when transitioning from invalid status', async () => {
      ...
      await expect(
        service.approve('verification-uuid', mockAdminId),
    ...
    it('should reject self-approval', async () => {
      const verificationRepo = {
        findOne: jest
          .fn()
          .mockResolvedValue(mockVerification({ providerId: mockAdminId })),
        save: jest.fn(),
      };
      service = await createModule({ verificationRepo });
  ...
  describe('reject', () => {
    it('should reject verification successfully', async () => {
      const verificationRepo = {
        ...
        save: jest
          .fn()
          .mockResolvedValue(mockVerification({ status: 'rejected' })),
      };
      const profileRepo = {
        findOne: jest.fn().mockResolvedValue({
          id: 'puuid',
    ...
    it('should throw BadRequestException when reason is missing', async () => {
      ...
      await expect(
        service.reject('verification-uuid', mockAdminId, ''),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('suspend', () => {
    it('should suspend an approved provider', async () => {
      const verificationRepo = {
        ...
        save: jest
          .fn()
          .mockResolvedValue(mockVerification({ status: 'suspended' })),
      };
      const profileRepo = {
        findOne: jest.fn().mockResolvedValue({
          id: 'puuid',
    ...
    it('should require suspension reason', async () => {
      ...
      await expect(
        service.suspend('verification-uuid', mockAdminId, ''),
      ).rejects.toThrow(BadRequestException);
    });
  });
